...
class DeefyRepository{
    ...
    public function getUserFromEmail($email) {
        ...
        // Debugging output
        //echo "<p>Hachage récupéré de la base de données pour l'email {$email} : {$userstmt['passwd']}</p>";
        
        return new User($userstmt['id'], $userstmt['email'], $userstmt['role'], $userstmt['passwd']);
    }
    ...
    public function playlistLinkTrack(Playlist $playlist, AudioTrack $track) {
        try {
            ...
            // Vérifier l'existence de la relation avant l'insertion
            $checkQuery = "SELECT * FROM playlist2track WHERE id_pl = :id_pl AND id_track = :id_track";
            $checkStmt = $this->pdo->prepare($checkQuery);
            $checkStmt->execute(['id_pl' => $playlistId, 'id_track' => $trackId]);
    
            if ($checkStmt->rowCount() === 0) {
                // Insérer la relation uniquement si elle n'existe pas déjà
                $query = "INSERT INTO playlist2track (id_pl, id_track) VALUES (:id_pl, :id_track)";
                $stmt = $this->pdo->prepare($query);
                $stmt->execute(['id_pl' => $playlistId, 'id_track' => $trackId]);
            } else {
                //echo "La piste est déjà liée à cette playlist.<br>";
            }
        } catch (\PDOException $e) {
